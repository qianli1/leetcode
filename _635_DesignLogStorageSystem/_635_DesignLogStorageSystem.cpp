...
class _635_DesignLogStorageSystem {
    /**
 ...
 public:
  ...
  vector<int> retrieve(string s, string e, string gra) {
    vector<int> res;
    int index = indexs[gra];
    for (auto ts : timestamps) {
        if (ts[1].substr(0, index) >= s.substr(0, index)
            && ts[1].substr(0, index) <= e.substr(0, index)) {
            res.push_back(stoi(ts[0]));
        }
    }
    return res;
  }
